...
@pytest.fixture
def long_get_mask_card_number() -> str:
    """Длинный номер карты"""
    return '1234567890123453123123123123'


@pytest.fixture
def empty_get_mask_card_number() -> str:
    """Пустой ввод"""
    return ''


@pytest.mark.parametrize("card_number, expected", [
    ('1234567890123453', "1234 56** **** 3453"),
    ('1234', "Неверный номер карты"),
    ('1234567890123453123123123123', "Неверный номер карты"),
...
@pytest.fixture
def check_stn_mask_account_card() -> str:
    """Стандартный номер счета"""
    return "Счет 73654108430135874305"


@pytest.fixture
def card_stn_mask_account_card() -> str:
    """Стандартный номер карты"""
    return "Visa 4108430135874305"


@pytest.fixture
def no_num_stn_mask_account_card() -> str:
    """Отсутствие номера"""
    return "Visa "


def test_check_stn_mask_account_card(check_stn_mask_account_card: str) -> None:
    """Проверка со стандартным номером счета"""
    assert mask_account_card(check_stn_mask_account_card) == "Счет **4305"


def test_card_stn_mask_account_card(card_stn_mask_account_card: str) -> None:
...
@pytest.fixture
def fix_get_date() -> str:
    """Тестовые данные даты"""
    return "2024-03-11T02:26:18.671407"


@pytest.fixture
def valid_date_without_ms() -> str:
...
def test_get_date(fix_get_date: str) -> None:
    ...
    assert get_date(fix_get_date) == "11.03.2024"  # исправлено: было assert "11.03.2024"


def test_get_date_without_ms(valid_date_without_ms: str) -> None:
    """Проверка на дату без миллисекунд"""
    assert get_date(valid_date_without_ms) == "11.03.2024"


def test_get_date_short(short_date: str) -> None:
...
@pytest.fixture
def no_matching_state() -> List[Dict[str, Any]]:
...
def test_filter_by_state_no_matching(no_matching_state: List[Dict[str, Any]]) -> None:
...
def test_filter_by_state_empty_list(empty_list: List[Any]) -> None:
...
def test_filter_by_state_parametrized_ids(
    sample_transactions: List[Dict[str, Any]],
    test_state: str,
...
@pytest.fixture
def same_dates_transactions() -> List[Dict[str, Any]]:
...
def test_sort_by_date_same_dates(same_dates_transactions: List[Dict[str, Any]]) -> None:
...
def test_sort_by_date_empty_list(empty_list: List[Any]) -> None:
    """Пустой список на входе
    Должен вернуться пустой список"""
...
def test_sort_by_date_parametrized_last(
    sample_transactions: List[Dict[str, Any]],
    descending: bool,
